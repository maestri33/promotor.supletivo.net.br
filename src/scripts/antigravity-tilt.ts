...
export function initAntigravityTilt(): void {
  // Verificação de acessibilidade e recursos do dispositivo
  ...
  tiltElements.forEach((el) => {
    ...
    const handleMove = (e: PointerEvent) => {
      ...
      // Mapeia para [-maxTilt, +maxTilt]
      // Inversão: mover para o topo inclina rotateX positivo
      state.targetX = (0.5 - y) * state.maxTilt * 2;
      state.targetY = (x - 0.5) * state.maxTilt * 2;

      // Atualiza coordenadas para spotlight
  ...
  function render() {
    ...
    for (const s of states) {
      ...
      if (Math.abs(dx) > 0.01 || Math.abs(dy) > 0.01 || s.isHovered) {
        ...
        // Animar subcamadas com profundidade 3D desacoplada
